...
detector_t YOLO_Init(char *cfgfile, char *weightfile)
{
    ...
    detector.boxes = calloc(l.w*l.h*l.n, sizeof(box));
    detector.probs = calloc(l.w*l.h*l.n, sizeof(float *));
    int j=0;
    for(j = 0; j < l.w*l.h*l.n; ++j) detector.probs[j] = calloc(l.classes + 1, sizeof(float *));
...
bbox_v YOLO_Process(detector_t detector,image im,float thresh)
{
	...
	/*
	list *options = read_data_cfg("/home/lucky/PycharmProjects/cleaner02/data/darknet/cfg/tiny-yolo-voc.cfg");
	char *name_list = option_find_str(options, "names", "/home/lucky/PycharmProjects/cleaner02/data/darknet/data/voc.names");
...
int ObsDetInstance_Process(ObsDetInstance* aInstance,bbox_v bbox_info)
{
	...
	for(i=0;i<bbox_info.bbox_count;i++)
	{
		...
		ObjRoi.x=bbox_info.bboxptr[i].x;
		ObjRoi.y=bbox_info.bboxptr[i].y;
		ObjRoi.w=bbox_info.bboxptr[i].w;
		ObjRoi.h=bbox_info.bboxptr[i].h;
		if(ObjRoi.w&&ObjRoi.h)
		{
			...
			candidate_board_line.lineheight=ObjRoi.h;
    ...
    if(board_line.linelength>0)
    {
	    ...
	    aInstance->Thing.x=board_line.u;
